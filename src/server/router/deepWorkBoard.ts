...
import { formatTaskFromPrisma, formatStepFromPrisma } from '../../shared/deep-work-formatters'
import { validateEdgeCreation } from '../../shared/deep-work-clustering'
import { buildConnectMorphResult, buildDisconnectMorphResult, classifyEdgeType } from '../../shared/deep-work-morph'
...
// formatTaskFromPrisma and formatStepFromPrisma are imported from shared/deep-work-formatters.ts
 ...
 */
export function deriveEdgesFromHydratedNodes(nodes: DeepWorkNodeWithData[]): DeepWorkEdge[] {
...
async function executeMorphResult(
  ...
  await prisma.$transaction(async (tx) => {
    // Apply the shared Task/Step structural mutations (store-agnostic engine).
    await applyTaskStructureMorph(tx, morphResult)

    // Apply this surface's projection update: swap DeepWorkNode identities (taskId ↔ stepId).
