...
import { Logger, defaultLogger } from '../types.js'
...
export class SpeechService {
  ...
  async transcribeAudio(audioFilePath: string, options?: {
    ...
    try {
      ...
      // Archive audio file if not already in tmp
      if (!audioFilePath.includes('/tmp/work-planner-audio')) {
        ...
        this.logger.debug(`Audio file archived to: ${archivePath}`)
      }
  ...
  async transcribeAudioBuffer(
    ...
    try {
      ...
      this.logger.debug(`Audio file saved to: ${tempFilePath}`)

      const result = await this.transcribeAudio(tempFilePath, options)
      return { ...result, savedPath: tempFilePath }
    } catch (error) {
      this.logger.error('Error transcribing audio buffer:', { error })
      if (error instanceof Error) {
   ...
   */
  getBrainstormingSettings(): { language: string; prompt: string } {
    return {
      language: 'en',
   ...
   */
  getWorkflowSettings(): { language: string; prompt: string } {
    return {
      language: 'en',
   ...
   */
  getSchedulingSettings(): { language: string; prompt: string } {
    return {
      language: 'en',
...
export const getSpeechService = (logger?: Logger): SpeechService => {
  if (!speechServiceInstance) {
    const apiKey = process.env.OPENAI_API_KEY
    if (!apiKey) {
      throw new Error('OPENAI_API_KEY environment variable is not set')
    }
    speechServiceInstance = new SpeechService(apiKey, logger)
  }
